...
export default function TodoSearchBar({ addData }) {
  ...
  return (
    <div className={style.container}>
      ...
      />
      <button className={style.submitButton} type="button" onClick={onClick}>
        등록
      </button>
      <ul
        className={`${style.recentList} ${recentSearchVisiblity ? "" : style.hidden}`}
      >
        {recentSearchState.length > 0 &&
          recentSearchState.map(({ keyword, id }) => {
            return (
              <li
                className={style.recentListItem}
                key={id}
                onClick={() => {
